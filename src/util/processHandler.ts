...
export class ProcessHandler {
  ...
  private state: ProcessHandlerState
  ...
  public async stopApp() {
    return new Promise(resolve => {
      if (this.state.appRunning) {
        console.log(chalk.dim('Sending SIGTERM signal to app...'))
  ...
  public runApp() {
    ...
    proc.on('error', e => {
      console.log(chalk.red.dim('Failed to start app: ' + e.message))
      this.setState({
        appRunning: false,
        proc: null,
        pid: null,
    ...
    proc.on('exit', (code, signal) => {
      if (code !== null) {
        console.log(
          code > 0
            ? chalk.red.dim('App exited with code ' + code + '.')
            : chalk.green.dim('App exited with code ' + code + '.')
        )
      }
      if (signal !== null) {
        console.log(
          signal !== 'SIGTERM'
            ? chalk.red.dim('App killed with signal ' + signal + '.')
            : chalk.green.dim('App killed with signal SIGTERM.')
        )
      }
      this.setState({
        appRunning: false,
    ...
    console.log(chalk.green.dim('App started!'))
  ...
  private setState(state: ProcessHandlerState) {
    this.state = state
  }
}
